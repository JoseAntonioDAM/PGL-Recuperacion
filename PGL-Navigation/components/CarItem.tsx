...
export function CarItem({ car, onDelete, onToggle }: CarItemProps) {
  return (
    ...
    >
      <Text style={styles.emoji}>{CATEGORIES[car.categoria].emoji}</Text>
      <View style={styles.info}>
        <Text style={[styles.nombre, car.marcado && styles.nombreTachado]}>
          {car.nombre}
        </Text>
        <Text style={styles.categoria}>{CATEGORIES[car.categoria].label}</Text>
        <Text style={styles.precio}>{car.precio.toLocaleString("es-ES")} €</Text>
      </View>
      <View style={styles.rightSection}>
        {car.marcado && <Text style={styles.checkmark}>✓</Text>}
